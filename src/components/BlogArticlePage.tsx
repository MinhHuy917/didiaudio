...
import { type Locale, withLocalePrefix } from '@/lib/i18n'
...
export default function BlogArticlePage({ slug, title, description, content, image, date, locale }: Props) {
  const articlePath = withLocalePrefix(locale, `/blog/${slug}`)
  const articleUrl = `https://www.didi-audio.com${articlePath}`
  const articleSchema = {
    ...
    <article className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black text-white" itemScope itemType="https://schema.org/Article">
      ...
      <header className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        <div className="flex items-center gap-3 text-sm text-gray-300 mb-6" aria-label="Breadcrumb">
          <Link href={withLocalePrefix(locale, '/')} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:border-cyan-500/50 transition-colors">
            <ArrowLeft className="w-4 h-4" />
            Home
          </Link>
          <span className="text-gray-500">/</span>
          <Link href={withLocalePrefix(locale, '/#blog')} className="text-gray-300 hover:text-cyan-400 transition-colors">
            Blog
          </Link>
          <span className="text-gray-500">/</span>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center mb-10 sm:mb-14">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/30">
              ...
              <span className="text-xs sm:text-sm font-semibold text-cyan-200">Blog • Audio & Events</span>
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-tight" itemProp="headline">
              <span className="bg-gradient-to-r from-cyan-400 via-purple-500 to-pink-500 bg-clip-text text-transparent">{title}</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <Link href={withLocalePrefix(locale, '/#products')} className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 font-semibold text-white hover:shadow-lg hover:shadow-cyan-500/40 transition-all">
                <Play className="w-4 h-4" />
                Book speaker now
              </Link>
              <Link href="tel:0339197917" className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white/5 border border-white/10 text-white hover:border-cyan-500/50 transition-all">
